...
class Controller:
    ...
    def handle_graph(self, e):
        ...
        self.fillDDProduct()
        self._view.txtOut.controls.append(ft.Text(f"Numero di vertici: {self._model.get_num_of_nodes()}     "
                                                  f"Numero di archi: {self._model.get_num_of_edges()}"))

        freq = {}
        for edge in self._model.get_sorted_edges()[:3]:
            self._view.txtOut.controls.append(ft.Text(f"L'arco da {edge[0].Product_number} a "
                                                      f"{edge[1].Product_number} ha peso {edge[2]['weight']}"))
        ...
        for k, v in freq.items():
            if v > 1:
                self._view.txtOut.controls.append(ft.Text(k))
        self._view.update_page()
