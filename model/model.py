...
class Model:
    ...
    def searchPath(self, product_number):
        nodoSorgente = self.idMap[product_number]
        parziale = []
        self.ricorsione(parziale, nodoSorgente, 0)
    ...
    def isNovel(self, e, parizale):
        if len(parizale) == 0:
            print("Parziale is empty in isNovel")
            return True
        e_inv = (e[1], e[0], e[2])
        return (e_inv not in parizale) and (e not in parizale)
...
FillDD con liste nel model e for nel controller
Query per gli archi con nodi e anno in input che restituisce il peso
Per ordinare gli archi in base al peso: sorted(self._grafo.edges(data=True), key=lambda x: x[2]['weight'], reverse=True)
Metodo nel controller per la frequenza degli archi di peso maggiore
